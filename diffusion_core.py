...
class SimpleConditionalUNet(nn.Module):
    def __init__(self, in_channels=3, base_dims=64, time_dim=128):
        ...
        # Fixed channel sizing for block4: concatenated x_up1 (base_dims*2) + h2 (base_dims*2) = base_dims*4 input channels
        self.block4 = ConditionalBlock(base_dims * 4, base_dims * 2, time_dim)

        self.up2 = nn.ConvTranspose2d(base_dims * 2, base_dims, 4, stride=2, padding=1)
        # Concatenated x_up2 (base_dims) + h1 (base_dims) = base_dims*2 input channels
        self.block5 = ConditionalBlock(base_dims * 2, base_dims, time_dim)
    ...
    def forward(self, x, t, text_embeddings):
        ...
        # Upsampling path with corrected skip-connection concatenations
        x_up1 = self.up1(h3)
